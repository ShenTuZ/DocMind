...
def toc_detector_single_page(content, model=None):
    try:
        print('start toc_detector_single_page')
        prompt = '''
        ...
        toc_detected = json_content.get('toc_detected', 'no')
        print(f'TOC detected: {toc_detected}')
        return toc_detected
    except Exception as e:
        print(f'Error in toc_detector_single_page: {e}')
        import traceback
        traceback.print_exc()
        return 'no'
...
def find_toc_pages(start_page_index, page_list, opt, logger=None):
    try:
        ...
        print(f'Checking pages from {start_page_index} to {min(len(page_list), opt.toc_check_page_num)}')
        
        while i < len(page_list):
            # Only check beyond max_pages if we're still finding TOC pages
            if i >= opt.toc_check_page_num and not last_page_is_yes:
                print(f'Stopping at page {i} because no TOC found yet')
                break
            
            print(f'Checking page {i} for TOC')
            try:
                detected_result = toc_detector_single_page(page_list[i][0], model=opt.model)
                print(f'Page {i} TOC detection result: {detected_result}')
                ...
                if detected_result == 'yes':
                    ...
                    print(f'Found last TOC page at {i-1}')
                    break
            except Exception as e:
                print(f'Error checking page {i} for TOC: {e}')
                if logger:
                    logger.error(f'Error checking page {i} for TOC: {e}')
        ...
        print(f'Error in find_toc_pages: {e}')
        if logger:
            logger.error(f'Error in find_toc_pages: {e}')
        import traceback
        traceback.print_exc()
        return []
...
def generate_toc_init(part, model=None):
    try:
        ...
        For the title, you need to extract the original title from the text, only fix the space inconsistency. The document is in Chinese, so make sure to extract Chinese titles correctly.
        ...
        print(f'Error in generate_toc_init: {e}')
        import traceback
        traceback.print_exc()
        raise
...
def process_no_toc(page_list, start_index=1, model=None, logger=None):
    try:
        logger.info('Starting process_no_toc')
        page_contents=[]
        token_lengths=[]
        ...
        logger.info(f'Page contents created: {len(page_contents)} pages')
        group_texts = page_list_to_group_text(page_contents, token_lengths)
        logger.info(f'len(group_texts): {len(group_texts)}')

        logger.info('Generating initial TOC')
        toc_with_page_number= generate_toc_init(group_texts[0], model)
        logger.info(f'Initial TOC generated: {len(toc_with_page_number)} items')
        
        for i, group_text in enumerate(group_texts[1:]):
            logger.info(f'Generating additional TOC for group {i+1}')
            toc_with_page_number_additional = generate_toc_continue(toc_with_page_number, group_text, model)
            logger.info(f'Additional TOC generated: {len(toc_with_page_number_additional)} items')
            toc_with_page_number.extend(toc_with_page_number_additional)
        
        logger.info(f'Final TOC generated: {len(toc_with_page_number)} items')
        logger.info(f'generate_toc: {toc_with_page_number}')

        logger.info('Converting physical index to int')
        toc_with_page_number = convert_physical_index_to_int(toc_with_page_number)
        logger.info(f'convert_physical_index_to_int: {toc_with_page_number}')

        return toc_with_page_number
    except Exception as e:
        logger.error(f'Error in process_no_toc: {e}')
        import traceback
...
async def meta_processor(page_list, mode=None, toc_content=None, toc_page_list=None, start_index=1, opt=None, logger=None):
    try:
        logger.info(f'Starting meta_processor with mode: {mode}')
        logger.info(f'start_index: {start_index}')
        
        if mode == 'process_toc_with_page_numbers':
            logger.info('Processing TOC with page numbers')
            ...
            logger.info('Processing TOC without page numbers')
            toc_with_page_number = process_toc_no_page_numbers(toc_content, toc_page_list, page_list, model=opt.model, logger=logger)
        else:
            logger.info('Processing without TOC')
            toc_with_page_number = process_no_toc(page_list, start_index=start_index, model=opt.model, logger=logger)
        
        logger.info(f'Initial TOC items: {len(toc_with_page_number)}')
            
        toc_with_page_number = [item for item in toc_with_page_number if item.get('physical_index') is not None] 
        logger.info(f'Filtered TOC items (non-None physical_index): {len(toc_with_page_number)}')
        
        logger.info('Validating and truncating physical indices')
        ...
        if accuracy == 1.0 and len(incorrect_results) == 0:
            logger.info('TOC verification passed with 100% accuracy')
            return toc_with_page_number
        if accuracy > 0.6 and len(incorrect_results) > 0:
            logger.info(f'TOC verification passed with {accuracy*100:.2f}% accuracy, fixing incorrect results')
            ...
            logger.info(f'TOC verification failed with {accuracy*100:.2f}% accuracy, trying next mode')
            if mode == 'process_toc_with_page_numbers':
        ...
        logger.error(f'Error in meta_processor: {e}')
        import traceback
        logger.error(traceback.format_exc())
        raise
...
async def tree_parser(page_list, opt, doc=None, logger=None):
    try:
        logger.info('Starting check_toc')
        check_toc_result = check_toc(page_list, opt)
        logger.info(check_toc_result)

        if check_toc_result.get("toc_content") and check_toc_result["toc_content"].strip() and check_toc_result["page_index_given_in_toc"] == "yes":
            logger.info('Processing with TOC and page numbers')
        ...
        logger.info('Adding preface if needed')
        toc_with_page_number = add_preface_if_needed(toc_with_page_number)
        logger.info('Checking title appearance in start')
        toc_with_page_number = await check_title_appearance_in_start_concurrent(toc_with_page_number, page_list, model=opt.model, logger=logger)
        
        # Filter out items with None physical_index before post_processings
        valid_toc_items = [item for item in toc_with_page_number if item.get('physical_index') is not None]
        logger.info(f'Filtered valid TOC items: {len(valid_toc_items)}')
        
        logger.info('Starting post_processing')
        toc_tree = post_processing(valid_toc_items, len(page_list))
        logger.info(f'Post processing completed, tree length: {len(toc_tree)}')
        
        logger.info('Processing large nodes recursively')
        tasks = [
        ...
        logger.error(f'Error in tree_parser: {e}')
        import traceback
        logger.error(traceback.format_exc())
        raise
...
def page_index_main(doc, opt=None):
    ...
    async def page_index_builder():
        try:
            logger.info('Starting tree parser')
            structure = await tree_parser(page_list, opt, doc=doc, logger=logger)
            logger.info('Tree parser completed')
            
            if opt.if_add_node_id == 'yes':
                logger.info('Adding node IDs')
                write_node_id(structure)
                logger.info('Node IDs added')
            
            if opt.if_add_node_text == 'yes':
                logger.info('Adding node text')
                add_node_text(structure, page_list)
                logger.info('Node text added')
            
            if opt.if_add_node_summary == 'yes':
                logger.info('Adding node summaries')
                if opt.if_add_node_text == 'no':
                    add_node_text(structure, page_list)
                await generate_summaries_for_structure(structure, model=opt.model)
                if opt.if_add_node_text == 'no':
                    remove_structure_text(structure)
                logger.info('Node summaries added')
                ...
                if opt.if_add_doc_description == 'yes':
                    ...
                    logger.info('Document description generated')
                    return {
            ...
            logger.error(f'Error in page_index_builder: {e}')
            import traceback
            logger.error(traceback.format_exc())
            raise
