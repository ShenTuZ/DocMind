...
def count_tokens(text, model=None):
    ...
    try:
        ...
        # 如果模型不在 tiktoken 的映射中（如 Ollama 模型），使用默认的分词器
    ...
    for i in range(max_retries):
        try:
            if USE_OLLAMA:
                ...
                return result["message"]["content"], "finished"
            else:
                # 使用 OpenAI API (SiliconFlow)
...
def extract_json(content):
    try:
        ...
        if start_idx != -1:
            ...
            # If no delimiters, look for the first '[' or '{' to find JSON start
            json_start = min(
            ...
            if json_start != float('inf'):
                ...
                if content[json_start] == '[':
                    ...
                    while count > 0 and json_end < len(content):
                        if content[json_end] == '[':
                            count += 1
                        elif content[json_end] == ']':
                            count -= 1
                        json_end += 1
                else:  # content[json_start] == '{'
                    # Find matching '}'
                    count = 1
                    json_end = json_start + 1
                    while count > 0 and json_end < len(content):
...
def get_page_tokens(pdf_path, model="gpt-4o-2024-11-20", pdf_parser="PyPDF2"):
    ...
    if pdf_parser == "PyPDF2":
        ...
        with tqdm(total=total_pages, desc="Extracting PDF pages") as pbar:
